# TODO (phase 2): Still writes chunks + embeddings to ChromaDB (data/chroma_db/ —
# gitignored, built locally, not reproducible from a clean clone). The production API
# (api/frag_nach.py) has already been migrated to pgvector (manifesto_chunks table).
# This script should write to pgvector instead. Low priority — runs manually, once
# per election cycle (~4 yrs).
